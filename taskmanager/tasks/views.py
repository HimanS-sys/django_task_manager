...
class TaskUpdateView(PermissionRequiredMixin, SprintTaskMixin, UpdateView):
    ...
    def has_permission(self):
        # check if user has general permission to edit the task
        has_general_permission = super().has_permission()
        if not has_general_permission:
            return False
        # check if user is either creator or owner of the task
...
class ContactFormView(FormView):
    ...
    def form_valid(self, form):
        ...
        form_email = form.cleaned_dat.get("form_email")
        send_contact_email(
            subject,
            message,
            form_email,
...
def claim_task_view(
    ...
    except TaskAlreadyClaimedException(Exception):
        return HttpResponse(
            "Task is already claimed orcompleted.",
            status = "400"
        )
...
def task_home(request):
    ...
    # Categorize tasks into their respective list
...
def manage_epic_tasks(request, epic_pk):
    ...
    if request.method == "POST":
        formset = EpicFormSet(
            request.POST,
            queryset = get_tasks_for_epic(epic)
        )
        if formset.is_valid():
            tasks = formset.save(commit = False)
            save_tasks_for_epic(epic, tasks)
            formset.save_m2m() # handles many to many relations if there are any
            return redirect("tasks:task-list")
    else:
        formset = EpicFormSet(queryset = get_tasks_for_epic(epic))
